...
export async function sendContactEmail(payload: ContactPayload) {
  ...
  const html = `
    <div style="font-family:system-ui,sans-serif;padding:24px;background:#0b0b12;color:#f2f2f5;border-radius:16px">
      ...
      <p style="color:#9b9ba8;font-size:12px">Sent via Luxe Portfolio</p>
    </div>`;

  await plunkSend({
    to: recipient,
    from: `Luxe Portfolio <${process.env.PLUNK_FROM ?? "no-reply@luxe.work"}>`,
    subject,
    body,
    html,
  });
  ...
  try {
    await plunkSend({
      to: payload.email,
      from: `Ariadne Voss <${process.env.PLUNK_FROM ?? "no-reply@luxe.work"}>`,
      subject: "Thanks for reaching out ✦",
      body: `Hi ${payload.name},\n\nThanks for your message — I read everything personally and will get back to you within 48 hours.\n\nBest,\nAriadne`,
      html: `<div style="font-family:system-ui,sans-serif;padding:24px;color:#181b26">
        <p>Hi ${escapeHtml(payload.name)},</p>
        <p>Thanks for your message — I read everything personally and will get back to you within 48 hours.</p>
        <p>Best,<br/>Ariadne</p>
      </div>`,
    });
  } catch {
